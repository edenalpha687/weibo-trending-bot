...
def buttons(update: Update, context: CallbackContext):
    ...
    # =========================
        ...
        if link:
            name_line = (
                f'<a href="{link}">'
                f'<b>{state.get("name", "Unknown Token")}</b>'
                f'</a>'
            )
        else:
            name_line = (
...
def messages(update: Update, context: CallbackContext):
    ...
    if state["step"] == "CA":
        ...
        if link:
            name_line = (
                f'<a href="{link}">'
                f'<b>{data.get("name", "Unknown Token")}</b>'
                f'</a>'
            )
        else:
            name_line = (
        ...
        # IMPORTANT:
        # Always convert to float before using
        # :,.2f or :,.0f formatting.
        liquidity = safe_float(
            data.get("liquidity")
        )
...
def main():
    ...
    # Register error handler so Telegram errors
    # don't produce "No error handlers are registered".
